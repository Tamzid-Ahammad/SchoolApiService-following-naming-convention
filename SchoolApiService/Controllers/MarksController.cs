...
namespace SchoolApiService.Controllers
{
	...
	public class MarksController : ControllerBase
	{
		...
		public async Task<IActionResult> PutMark(int id, Mark mark)
		{
			...
			if (mark.StudentId != null && !await _context.Students.AnyAsync(s => s.StudentId == mark.StudentId))
			{
			...
			if (mark.SubjectId != null && !await _context.Subjects.AnyAsync(s => s.SubjectId == mark.SubjectId))
			{
		...
		public async Task<ActionResult<Mark>> PostMark(Mark mark)
		{
			...
			{
				// Check if the provided StudentId exists in the database
				var existingStudent = await _context.Students.FindAsync(mark.StudentId);
				...
				// Check if the provided SubjectId exists in the database
				var existingSubject = await _context.Subjects.FindAsync(mark.SubjectId);
